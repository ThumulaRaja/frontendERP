...
function Header({
  ...
  // Use the useCookies hook to access USER_ID and USERNAME
  ...
  if (rememberedUser) {
    ...
    const { USER_ID, NAME, PHOTO } = rememberedUser;
    NAME1 = NAME;
    PHOTO1 = PHOTO;
  ...
  const handleLogout = () => {
    // Clear cookies and redirect to the sign-in page
    Cookies.remove('rememberedUser');
    window.location.href = '/';
  };
  ...
  const signInMenu = (
      <Menu
          ...
          style={{ minWidth: '100%', borderRadius: '8px', fontSize: '18px' }}
      >
        <Menu.Item key="1">
          <NavLink to="/profile">
        ...
        <Col span={24} md={18} className="header-control">
          ...
          <Dropdown overlay={signInMenu} trigger={['click']}>
            <Button type="link" className="btn-sign-in" style={{ fontSize: '15px', display: 'flex', alignItems: 'center' }}>
              <img
                  src={PHOTO1} // Assuming PHOTO1 is the source URL for the image
                  alt="Profile"
                  style={{ width: '30px', height: '30px', marginRight: '5px', borderRadius: '20%' }}
              />
              <span>{NAME1}</span>
            </Button>
